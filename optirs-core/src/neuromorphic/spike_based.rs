    ...
    > SpikingOptimizer<T>
{
    ...
    fn rate_decode(&self, spike_train: &SpikeTrain<T>) -> Result<T> {
        ...
        let spike_count =
            T::from(spike_train.spike_count).unwrap_or_else(|| T::zero());
        let rate = spike_count
            / (window_duration / T::from(1000.0).unwrap_or_else(|| T::zero()));
    ...
    fn weighted_spike_count_decode(&self, spike_train: &SpikeTrain<T>) -> Result<T> {
        ...
        for &spike_time in &spike_train.spike_times {
            ...
            let weight =
                (-time_diff / T::from(10.0).unwrap_or_else(|| T::zero())).exp(); // Exponential decay
    ...
    > SpikeTrainOptimizer<T>
{
    ...
    fn compute_pattern_similarity(
        ...
        let count_similarity = T::one()
            - T::from(count_diff / max_spikes as f64).unwrap_or_else(|| T::zero());
