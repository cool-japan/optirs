...
impl<A: Float + Send + Sync + Send + Sync> PredictionModel<A> {
    ...
    pub fn predict(
        ...
        for i in 0..horizon {
            ...
            if recent_data.len() >= self.model_order {
                let predicted_features = recent_data[0].features.clone();
    ...
    > StreamFusionOptimizer<A>
{
    ...
    pub fn fuse_optimization_steps(&mut self, steps: &[(String, Array1<A>)]) -> Result<Array1<A>> {
        ...
        match self.fusion_strategy {
            ...
            FusionStrategy::MedianFusion => Ok(steps[0].1.clone()),
...
impl<A: Float + Send + Sync + Send + Sync> PipelineExecutionManager<A> {
