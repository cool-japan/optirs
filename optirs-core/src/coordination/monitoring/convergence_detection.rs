...
impl<T: Float + Debug + Send + Sync + 'static> ConvergenceAnalyzer<T> {
    ...
    fn anderson_darling_test(&self, state: &ConvergenceState<T>) -> T {
        ...
        let kurt_score =
            (-(kurtosis - T::from(3.0).unwrap_or_else(|| T::zero())).abs()).exp();
    ...
    fn compute_cdf_max_difference(&self, first: &[T], second: &[T]) -> T {
        ...
        let steps = 20;
        let step_size =
            (max_val - min_val) / T::from(steps).unwrap_or_else(|| T::zero());
    ...
    fn assess_trend_convergence(&self, trend: &TrendAnalysis<T>) -> T {
        ...
        let r_squared_penalty =
            if trend.r_squared > T::from(0.8).unwrap_or_else(|| T::zero()) {
    ...
    fn linear_regression(&self, data: &VecDeque<T>) -> (T, T) {
        ...
        let ss_res = data.iter().enumerate().fold(T::zero(), |acc, (i, &y)| {
            let predicted = slope * T::from(i).unwrap_or_else(|| T::zero())
                + (sum_y - slope * sum_x) / n;
    ...
    fn compute_acceleration(&self, data: &VecDeque<T>) -> T {
        ...
        for i in 2..data.len() {
            let accel = data[i]
                - T::from(2.0).unwrap_or_else(|| T::zero()) * data[i - 1]
                + data[i - 2];
    ...
    fn compute_momentum(&self, data: &VecDeque<T>) -> T {
        ...
        if recent_window >= 2 {
            ...
            (end_val - start_val)
                / T::from(recent_window - 1).unwrap_or_else(|| T::zero())
        } else {
            T::zero()
        }
    }
