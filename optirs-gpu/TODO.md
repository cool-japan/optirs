...
**Tests**: 243 passing (`cargo nextest run --all-features`) + 2 passing doctests (`cargo test --doc --all-features`).
...
**Clippy**: 2 pre-existing `clippy::wrong_self_convention` lints on `to_gpu`/`to_cpu` (see "Known minor items" below); no other lints.
...
- [x] GPU/CPU parity integration tests (`tests/gpu_parity.rs`, 8 tests) dispatch on real hardware and assert agreement with a CPU reference optimizer within `1e-5`, including a 10-step trajectory (proves device-resident state and bias-correction step counting are both real) and a host round-trip mid-training (proves `to_cpu`/`to_gpu` genuinely move data, not just flip a flag).
...
- [x] Vendor memory-API shape (`memory/vendors/` — CUDA/ROCm/oneAPI/Metal pool/stream/statistics types over ordinary system-heap allocations; UB-free allocation path with a real `Layout` reconstructed at free time; explicitly documented as a Pure-Rust API-shape simulation, not real driver access).
...
- `GpuOptimizer::to_gpu`/`to_cpu` trip `clippy::wrong_self_convention` (the lint expects a consuming `self` for a `to_*` name; the trait's semantics are "move state to/from the device in place", which needs `&mut self`). Pre-existing, not a correctness issue; left as-is pending a deliberate naming decision rather than a silent rename of public API.
