...
impl<T: Float + Debug + Send + Sync + std::iter::Sum> ProgressiveNAS<T> {
        ...
        match self.progression_strategy {
            ...
            ProgressionStrategy::Adaptive => {
                ...
                if let Some(trend) = self.analyze_performance_trend() {
                    if trend < scirs2_core::numeric::NumCast::from(0.01).unwrap_or_else(|| T::zero())
    ...
    fn analyze_performance_trend(&self) -> Option<T> {
        ...
        let sum_x = n * (n - T::one()) / scirs2_core::numeric::NumCast::from(2).unwrap_or_else(|| T::zero());
    ...
    pub fn get_current_search_config(&self) -> SearchPhaseConfig<T> {
        match self.current_phase {
            SearchPhase::Initial => SearchPhaseConfig {
                complexity_limit: scirs2_core::numeric::NumCast::from(0.25).unwrap_or_else(|| T::zero()),
                max_components: 2,
                max_depth: 3,
                exploration_factor: scirs2_core::numeric::NumCast::from(0.8).unwrap_or_else(|| T::zero()),
                mutation_rate: scirs2_core::numeric::NumCast::from(0.3).unwrap_or_else(|| T::zero()),
            ...
            SearchPhase::Intermediate => SearchPhaseConfig {
                complexity_limit: scirs2_core::numeric::NumCast::from(0.5).unwrap_or_else(|| T::zero()),
                max_components: 4,
                max_depth: 5,
                exploration_factor: scirs2_core::numeric::NumCast::from(0.6).unwrap_or_else(|| T::zero()),
                mutation_rate: scirs2_core::numeric::NumCast::from(0.2).unwrap_or_else(|| T::zero()),
            ...
            SearchPhase::Advanced => SearchPhaseConfig {
                complexity_limit: scirs2_core::numeric::NumCast::from(0.75).unwrap_or_else(|| T::zero()),
                max_components: 6,
                max_depth: 7,
                exploration_factor: scirs2_core::numeric::NumCast::from(0.4).unwrap_or_else(|| T::zero()),
                mutation_rate: scirs2_core::numeric::NumCast::from(0.15).unwrap_or_else(|| T::zero()),
            ...
            SearchPhase::Final => SearchPhaseConfig {
                ...
                max_components: 8,
                max_depth: 10,
                exploration_factor: scirs2_core::numeric::NumCast::from(0.2).unwrap_or_else(|| T::zero()),
...
impl<T: Float + Debug + Send + Sync + 'static> ComplexityScheduler<T> {
    ...
    pub fn new() -> Result<Self> {
        Ok(Self {
            current_complexity: scirs2_core::numeric::NumCast::from(0.1).unwrap_or_else(|| T::zero()),
