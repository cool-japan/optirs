    ...
    T: From<f64> + std::fmt::Debug,
{
    fn get_current_usage(&self) -> Result<ResourceUsage<T>> {
        ...
        Ok(ResourceUsage {
            memory_gb: scirs2_core::numeric::NumCast::from(used_memory).unwrap_or_else(|| T::zero()),
            ...
            peak_memory_gb: scirs2_core::numeric::NumCast::from(used_memory * 1.2).unwrap_or_else(|| T::zero()), // 20% overhead
    ...
    fn get_system_metrics(&self) -> Result<SystemMetrics<T>> {
        ...
        Ok(SystemMetrics {
            ...
            available_disk_gb: scirs2_core::numeric::NumCast::from(total_disk - used_disk)
                .unwrap_or_else(|| T::zero()),
            network_bandwidth: scirs2_core::numeric::NumCast::from(bandwidth).unwrap_or_else(|| T::zero()),
            system_temperature: scirs2_core::numeric::NumCast::from(temperature).unwrap_or_else(|| T::zero()),
            power_consumption: scirs2_core::numeric::NumCast::from(power).unwrap_or_else(|| T::zero()),
        })
    }
    ...
    fn check_limits(
        ...
        // Check memory limit
        let memory_limit = scirs2_core::numeric::NumCast::from(constraints.hardware_resources.max_memory_gb)
            .unwrap_or_else(|| T::zero());
    ...
    fn optimize(
            ...
            > memory_limit * scirs2_core::numeric::NumCast::from(0.8).unwrap_or_else(|| T::zero())
        {
            let reduction_target = current_usage.memory_gb
                * scirs2_core::numeric::NumCast::from(self.aggressiveness * 0.2).unwrap_or_else(|| T::zero());

            let mut parameters = HashMap::new();
            parameters.insert("memory_reduction_gb".to_string(), reduction_target);
            parameters.insert(
                "aggressiveness".to_string(),
                scirs2_core::numeric::NumCast::from(self.aggressiveness).unwrap_or_else(|| T::zero()),
            );
            ...
            Ok(OptimizationAction {
                ...
                expected_savings: ResourceUsage {
                    ...
                    cpu_time_seconds: scirs2_core::numeric::NumCast::from(0.0).unwrap_or_else(|| T::zero()),
                    gpu_time_seconds: scirs2_core::numeric::NumCast::from(0.0).unwrap_or_else(|| T::zero()),
                    energy_kwh: scirs2_core::numeric::NumCast::from(0.0).unwrap_or_else(|| T::zero()),
                    network_io_gb: scirs2_core::numeric::NumCast::from(0.0).unwrap_or_else(|| T::zero()),
                    disk_io_gb: scirs2_core::numeric::NumCast::from(0.0).unwrap_or_else(|| T::zero()),
                    peak_memory_gb: reduction_target,
                    efficiency_score: scirs2_core::numeric::NumCast::from(0.1).unwrap_or_else(|| T::zero()),
                    cost_usd: scirs2_core::numeric::NumCast::from(0.0).unwrap_or_else(|| T::zero()),
                    network_gb: scirs2_core::numeric::NumCast::from(0.0).unwrap_or_else(|| T::zero()),
                },
                implementation_cost: scirs2_core::numeric::NumCast::from(0.05).unwrap_or_else(|| T::zero()),
            ...
            Ok(OptimizationAction {
                ...
                implementation_cost: scirs2_core::numeric::NumCast::from(0.0).unwrap_or_else(|| T::zero()),
    ...
    T: From<f64> + std::fmt::Display,
{
    ...
    pub fn update_usage(&self) -> Result<()> {
        ...
        if let Some(metrics) = system_metrics {
            let snapshot = ResourceSnapshot {
                ...
                cpu_load_average: scirs2_core::numeric::NumCast::from(0.6).unwrap_or_else(|| T::zero()),
                gpu_utilization: scirs2_core::numeric::NumCast::from(0.8).unwrap_or_else(|| T::zero()),
            };
    ...
    fn calculate_metric_trend(&self, values: Vec<T>) -> TrendDirection {
        ...
        let change_threshold = scirs2_core::numeric::NumCast::from(0.05).unwrap_or_else(|| T::zero()); // 5% change threshold
