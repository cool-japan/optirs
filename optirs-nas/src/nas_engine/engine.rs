    ...
    > NeuralArchitectureSearch<T>
{
    ...
    fn calculate_resource_usage(
        ...
        let component_count =
            scirs2_core::numeric::NumCast::from(architecture.components.len()).unwrap_or_else(|| T::zero());
        let connection_count =
            scirs2_core::numeric::NumCast::from(architecture.connections.len()).unwrap_or_else(|| T::zero());

        // Simple resource estimation model
        let memory_gb = component_count * scirs2_core::numeric::NumCast::from(0.1).unwrap_or_else(|| T::zero())
            + connection_count * scirs2_core::numeric::NumCast::from(0.05).unwrap_or_else(|| T::zero());
        let cpu_time = component_count * scirs2_core::numeric::NumCast::from(1.0).unwrap_or_else(|| T::zero())
            + connection_count * scirs2_core::numeric::NumCast::from(0.5).unwrap_or_else(|| T::zero());
        let gpu_time = component_count * scirs2_core::numeric::NumCast::from(0.5).unwrap_or_else(|| T::zero());
        let energy_kwh =
            (cpu_time + gpu_time) * scirs2_core::numeric::NumCast::from(0.001).unwrap_or_else(|| T::zero());
        let cost_usd = energy_kwh * scirs2_core::numeric::NumCast::from(0.12).unwrap_or_else(|| T::zero()); // $0.12 per kWh
    ...
    fn update_search_statistics(&mut self) {
        ...
        if !self.search_history.is_empty() {
            ...
            self.search_statistics.population_diversity =
                scirs2_core::numeric::NumCast::from(self.calculate_population_diversity(&recent_results))
            ...
            if !scores.is_empty() {
                ...
                self.search_statistics.average_score =
                    sum / scirs2_core::numeric::NumCast::from(scores.len()).unwrap_or_else(|| T::one());
            }
        }
    }
    ...
    fn extract_convergence_data(&self) -> ConvergenceData<T> {
        ...
        for generation in 0..=self.current_generation {
            ...
            if !generation_results.is_empty() {
                ...
                diversity_over_time
                    .push(scirs2_core::numeric::NumCast::from(diversity).unwrap_or_else(|| T::zero()));
            }
        }
        ...
        ConvergenceData {
            ...
            stability_measure: if diversity_over_time.len() > 1 {
                ...
                let variance = recent_diversity
                    ...
                    / scirs2_core::numeric::NumCast::from(recent_diversity.len()).unwrap_or_else(|| T::one());
                variance.sqrt()
            } else {
                T::one()
            },
    ...
    fn create_evaluation_summary(&self) -> EvaluationSummary<T> {
        ...
        let success_rate = if total_evaluations > 0 {
            scirs2_core::numeric::NumCast::from(successful_evaluations as f64 / total_evaluations as f64)
                .unwrap_or_else(|| T::zero())
        } else {
            T::zero()
        };
...
impl<T: Float + Debug + Send + Sync + 'static> PerformancePredictor<T> {
    pub fn new(_config: &EvaluationConfig<T>) -> Result<Self> {
        Ok(Self {
            ...
            prediction_accuracy: scirs2_core::numeric::NumCast::from(0.8).unwrap_or_else(|| T::zero()),
