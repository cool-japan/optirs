    ...
    PerformanceEvaluator<T>
{
    ...
    fn compute_stability(&self, results: &[TestResult<T>]) -> Result<T> {
        ...
        let cv = std_dev
            / mean
                .abs()
                .max(num_traits::cast::cast(1e-6).unwrap_or_else(|| T::zero()));
...
impl<T: Float + Debug + Default + Send + Sync> BenchmarkSuite<T> {
    ...
    fn add_standard_benchmarks(&mut self) -> Result<()> {
        ...
        self.standard_benchmarks.push(StandardBenchmark {
            ...
            expected_range: (
                num_traits::cast::cast(1e-8).unwrap_or_else(|| T::zero()),
                num_traits::cast::cast(1e-2).unwrap_or_else(|| T::zero()),
            ),
        ...
        self.standard_benchmarks.push(StandardBenchmark {
            ...
            expected_range: (
                num_traits::cast::cast(1e-10).unwrap_or_else(|| T::zero()),
                num_traits::cast::cast(1e-4).unwrap_or_else(|| T::zero()),
            ),
...
impl<T: Float + Debug + Default + std::iter::Sum + Send + Sync> StatisticalAnalyzer<T> {
    ...
    fn compute_confidence_intervals(
        ...
        if !results.is_empty() {
            ...
            let margin = std_dev * num_traits::cast::cast(1.96).unwrap_or_else(|| T::zero())
                / T::from((scores.len() as f64).sqrt()).unwrap();
            intervals.insert(
    ...
    fn new() -> Self {
        Self {
            ...
            limits: ResourceLimits {
                ...
                max_evaluation_time_seconds: num_traits::cast::cast(3600.0)
