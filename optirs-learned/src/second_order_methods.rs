...
use optirs_core::optimizers::{Optimizer, OptimizerState};
...
impl<T: Float + Debug + Default + Clone + Send + Sync + 'static> NewtonOptimizer<T> {
...
impl<T: Float + Debug + Default + Clone + Send + Sync + 'static> QuasiNewtonOptimizer<T> {
...
pub mod utils {
    ...
    pub fn compute_condition_number<T: Float + Debug + Send + Sync + 'static>(hessian: &HessianMatrix<T>) -> Option<T> {
    ...
    pub fn add_damping<T: Float + Debug + Send + Sync + 'static>(hessian: &mut Array2<T>, damping: T) {
    ...
    pub fn compute_optimal_damping<T: Float + Debug + Send + Sync + 'static>(
