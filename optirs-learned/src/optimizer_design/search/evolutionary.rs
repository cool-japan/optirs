...
#[derive(Debug)]
pub struct EvolutionarySearchState<T: Float + Debug + Send + Sync + 'static> {
    /// Current population
...
impl<T: Float + Debug + Default + std::fmt::Debug + Send + Sync> EvolutionarySearchState<T> {
...
pub struct NSGA2State<T: Float + Debug + Send + Sync + 'static> {
    /// Population
    pub population: Vec<ArchitectureCandidate>,
...
impl<T: Float + Debug + Default + std::fmt::Debug + Send + Sync> NSGA2State<T> {
