...
impl<T: Float + Debug + Send + Sync + 'static> AdaptiveSequenceProcessor<T> {
    ...
    fn update_importance_scores(&mut self, analysis: &LandscapeAnalysis<T>) -> Result<()> {
        ...
        let complexity_boost =
            analysis.complexity * scirs2_core::numeric::NumCast::from(0.3).unwrap_or_else(|| T::zero());
        let difficulty_boost =
            analysis.difficulty * scirs2_core::numeric::NumCast::from(0.2).unwrap_or_else(|| T::zero());
            ...
            for i in 0..seq_len {
                for j in 0..seq_len {
                    ...
                    let base_attention = (-scirs2_core::numeric::NumCast::from(distance)
                        .unwrap_or_else(|| T::zero())
                        / (scirs2_core::numeric::NumCast::from(seq_len).unwrap_or_else(|| T::zero())
                    ...
                    patterns[[head, i, j]] =
                        scirs2_core::numeric::NumCast::from(modulated_attention).unwrap_or_else(|| T::zero());
                }
            }
        }
