...
impl<T: Float + Debug + Default + Clone + Send + Sync + 'static> LearningRateAdapter<T> {
    ...
    fn exponential_decay(&self) -> T {
        let steps = scirs2_core::numeric::NumCast::from(self.step_count as f64).unwrap_or_else(|| T::zero());
        let decay_steps = scirs2_core::numeric::NumCast::from(self.adaptation_params.decay_steps as f64)
            .unwrap_or_else(|| T::zero());
    ...
    fn cosine_annealing(&self) -> T {
        let steps = scirs2_core::numeric::NumCast::from(self.step_count as f64).unwrap_or_else(|| T::zero());
        let total_steps = scirs2_core::numeric::NumCast::from(self.adaptation_params.decay_steps as f64)
            .unwrap_or_else(|| T::zero());
    ...
    fn warm_restart(&self) -> T {
        ...
        let progress = scirs2_core::numeric::NumCast::from(cycle_position as f64).unwrap_or_else(|| T::zero())
        ...
        if let Some(current_loss) = loss {
            if let Some(best_loss) = self.best_loss {
                ...
                if improvement > self.adaptation_params.improvement_threshold {
                    ...
                    Ok(self.current_lr * scirs2_core::numeric::NumCast::from(1.01).unwrap_or_else(|| T::zero()))
                } else {
    ...
    fn gradient_adaptive(&self, gradients: Option<&Array1<T>>) -> Result<T> {
        if let Some(grad) = gradients {
            ...
            let scale_factor = target_norm
                / (grad_norm + scirs2_core::numeric::NumCast::from(1e-8).unwrap_or_else(|| T::zero()));
