    ...
    > TransformerRegularizer<T>
{
    ...
    pub fn apply_regularization(
        ...
        match self.strategy {
            ...
            RegularizationStrategy::SpectralNorm => {
                self.apply_spectral_normalization(parameters, gradients)
            }
    ...
    fn compute_adaptive_strength(&self, param_name: &str) -> Result<T> {
        if let Some(stats) = self.parameter_stats.get(param_name) {
            ...
            let step_decay = decay_rate.powf(
    ...
    fn compute_spectral_norm_static(
        ...
        if !spectral_state.u_vectors.contains_key(param_name) {
            spectral_state
                .u_vectors
                .insert(param_name.to_string(), Array1::ones(m));
            spectral_state
                .v_vectors
                .insert(param_name.to_string(), Array1::ones(n));
        }
