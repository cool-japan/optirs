    ...
    > GradientProcessor<T>
{
    ...
    fn adaptive_scale_gradients(&self, gradients: &Array1<T>) -> Result<Array1<T>> {
        ...
        if mean_norm > T::zero() {
            let adaptive_scale =
                scirs2_core::numeric::NumCast::from(0.9).unwrap_or_else(|| T::zero()) * mean_norm / current_norm
    ...
    fn accumulate_gradients(&mut self, gradients: &Array1<T>) -> Result<Array1<T>> {
        ...
        if self.gradient_stats.update_count % self.processing_params.accumulation_steps == 0 {
            if let Some(accumulated) = self.accumulated_gradients.take() {
                let scale =
                    scirs2_core::numeric::NumCast::from(1.0 / self.processing_params.accumulation_steps as f64)
...
impl<T: Float + Debug + Default + Clone + Send + Sync + 'static> GradientStatistics<T> {
    ...
    pub fn new() -> Self {
        Self {
            ...
            min_magnitude: scirs2_core::numeric::NumCast::from(f64::INFINITY).unwrap_or_else(|| T::zero()),
            update_count: 0,
            sparsity: T::zero(),
        }
    }
    ...
    pub fn update(&mut self, gradients: &Array1<T>) {
        ...
        self.update_count += 1;
        let count = scirs2_core::numeric::NumCast::from(self.update_count as f64).unwrap_or_else(|| T::zero());
        ...
        let zero_count = gradients
            .iter()
            .filter(|&&x| x.abs() < scirs2_core::numeric::NumCast::from(1e-8).unwrap_or_else(|| T::zero()))
            .count();
