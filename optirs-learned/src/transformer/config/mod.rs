...
/// Attention mechanism configuration
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AttentionConfig<T: Float + Debug + Send + Sync + 'static> {
...
impl<T: Float + Debug + Send + Sync + 'static> Default for AttentionConfig<T> {
    fn default() -> Self {
        Self {
            head_dim: 64,
            scaled: true,
...
/// Training configuration
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TrainingConfig<T: Float + Debug + Send + Sync + 'static> {
    /// Learning rate
    pub learning_rate: T,
    /// Optimizer type
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GradientClippingConfig<T: Float + Debug + Send + Sync + 'static> {
    /// Clipping method
...
impl<T: Float + Debug + Send + Sync + 'static> Default for GradientClippingConfig<T> {
    fn default() -> Self {
        Self {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum ClippingMethod<T: Float + Debug + Send + Sync + 'static> {
    /// Clip by norm
    Norm,
    /// Clip by value
    Value,
