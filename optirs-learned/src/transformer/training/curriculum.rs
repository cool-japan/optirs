...
impl<T: Float + Debug + Send + Sync + 'static + Default + Clone> CurriculumLearner<T> {
    ...
    fn adapt_curriculum_parameters(&mut self, task_id: &str, performance: T) -> Result<()> {
        ...
        if performance_variance > scirs2_core::numeric::NumCast::from(0.1).unwrap_or_else(|| T::zero()) {
    ...
    fn get_expected_performance(&self) -> T {
        ...
        let difficulty_factor =
            self.curriculum_state.current_difficulty / self.curriculum_params.max_difficulty;
        T::one() - difficulty_factor * scirs2_core::numeric::NumCast::from(0.5).unwrap_or_else(|| T::zero())
    }
...
impl<T: Float + Debug + Send + Sync + 'static + Default + Clone> CurriculumState<T> {
    fn new() -> Result<Self> {
        Ok(Self {
            current_difficulty: scirs2_core::numeric::NumCast::from(0.1).unwrap_or_else(|| T::zero()),
...
impl<T: Float + Debug + Send + Sync + 'static + Default + Clone> Default for CurriculumParams<T> {
    fn default() -> Self {
        Self {
            initial_difficulty: scirs2_core::numeric::NumCast::from(0.1).unwrap_or_else(|| T::zero()),
            max_difficulty: scirs2_core::numeric::NumCast::from(1.0).unwrap_or_else(|| T::zero()),
            difficulty_increment: scirs2_core::numeric::NumCast::from(0.05).unwrap_or_else(|| T::zero()),
            progression_threshold: scirs2_core::numeric::NumCast::from(0.8).unwrap_or_else(|| T::zero()),
            patience: 5,
            self_pacing_factor: scirs2_core::numeric::NumCast::from(0.1).unwrap_or_else(|| T::zero()),
            diversity_weight: scirs2_core::numeric::NumCast::from(0.2).unwrap_or_else(|| T::zero()),
            teacher_confidence: scirs2_core::numeric::NumCast::from(0.9).unwrap_or_else(|| T::zero()),
        }
    }
}
