...
impl<T: Float + Debug + Send + Sync + 'static> EmbeddingLayer<T> {
...
impl<T: Float + Debug + Send + Sync + 'static> LayerNormalization<T> {
...
impl DropoutLayer {
    ...
    pub fn forward<T: Float + Debug + Send + Sync + 'static>(
        ...
        let scale = scirs2_core::numeric::NumCast::from(1.0 / keep_prob).unwrap_or_else(|| T::zero());
...
pub struct ResidualConnections<T: Float + Debug + Send + Sync + 'static> {
    /// Model dimension
    dimension: usize,
...
impl<T: Float + Debug + Send + Sync + 'static> ResidualConnections<T> {
...
impl ActivationLayer {
    ...
    /// ReLU activation
    fn relu<T: Float + Debug + Send + Sync + 'static>(input: &Array2<T>) -> Array2<T> {
    ...
    fn gelu<T: Float + Debug + Send + Sync + 'static>(input: &Array2<T>) -> Array2<T> {
        input.map(|&x| {
            ...
            let one = T::one();
            let sqrt_2_pi = scirs2_core::numeric::NumCast::from(0.797884560802865).unwrap_or_else(|| T::zero()); // sqrt(2/π)
    ...
    /// Swish activation
    fn swish<T: Float + Debug + Send + Sync + 'static>(input: &Array2<T>) -> Array2<T> {
        input.map(|&x| x * Self::sigmoid_scalar(x))
    }

    /// Tanh activation
    fn tanh<T: Float + Debug + Send + Sync + 'static>(input: &Array2<T>) -> Array2<T> {
        input.map(|&x| x.tanh())
    }

    /// Sigmoid activation
    fn sigmoid<T: Float + Debug + Send + Sync + 'static>(input: &Array2<T>) -> Array2<T> {
    ...
    fn sigmoid_scalar<T: Float + Debug + Send + Sync + 'static>(x: T) -> T {
        let one = T::one();
        one / (one + (-x).exp())
    }
}
