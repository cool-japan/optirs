    ...
    fn initialize_sinusoidal(&mut self) -> Result<()> {
        for pos in 0..self.max_sequence_length {
            for i in 0..self.model_dimension {
                let position = scirs2_core::numeric::NumCast::from(pos).unwrap_or_else(|| T::zero());
                let dimension = scirs2_core::numeric::NumCast::from(i).unwrap_or_else(|| T::zero());
                let model_dim =
                    scirs2_core::numeric::NumCast::from(self.model_dimension).unwrap_or_else(|| T::zero());

                let angle = position
                    / scirs2_core::numeric::NumCast::from(10000.0)
                        .unwrap_or_else(|| T::zero())
                        .powf(
                            scirs2_core::numeric::NumCast::from(2.0).unwrap_or_else(|| T::zero()) * dimension
                                / model_dim,
                        );
    ...
    pub fn sinusoidal_with_base(
        ...
        for pos in 0..max_sequence_length {
            for i in 0..model_dimension {
                ...
                let dimension = scirs2_core::numeric::NumCast::from(i).unwrap_or_else(|| T::zero());
                let model_dim =
                    scirs2_core::numeric::NumCast::from(model_dimension).unwrap_or_else(|| T::zero());
...
pub struct RelativePositionalEncoding<T: Float + Debug + Send + Sync + 'static> {
...
impl<T: Float + Debug + Send + Sync + 'static> RelativePositionalEncoding<T> {
    ...
    pub fn initialize_sinusoidal(&mut self) -> Result<()> {
        ...
        for i in 0..table_size {
            ...
            let position = scirs2_core::numeric::NumCast::from(relative_pos).unwrap_or_else(|| T::zero());
