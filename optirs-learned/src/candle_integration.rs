...
pub struct CandleTensor<T: Float + Debug + Send + Sync + 'static> {
    /// Underlying data
    data: Array1<T>,
...
pub struct CandleOptimizer<T: Float + Debug + Send + Sync + 'static> {
    /// AutoDiff engine
...
impl<T: Float + Debug + Default + Clone + Send + Sync + 'static> CandleTensor<T> {
...
impl<T: Float + Debug + Default + Clone + Send + Sync + 'static> CandleOptimizer<T> {
...
/// Saved tensor state
#[derive(Debug, Clone)]
pub struct TensorState<T: Float + Debug + Send + Sync + 'static> {
    pub data: Array1<T>,
...
#[derive(Debug, Clone)]
pub struct CandleOptimizerState<T: Float + Debug + Send + Sync + 'static> {
...
pub mod utils {
    ...
    pub fn same_device<T: Float + Debug + Send + Sync + 'static>(tensor1: &CandleTensor<T>, tensor2: &CandleTensor<T>) -> bool {
