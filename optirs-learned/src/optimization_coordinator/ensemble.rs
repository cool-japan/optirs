...
#[derive(Debug)]
pub struct OptimizerEnsemble<T: Float + Debug + Send + Sync + 'static> {
    /// Active optimizers
...
impl<T: Float + Debug + std::fmt::Debug + Send + Sync + std::default::Default + 'static> OptimizerEnsemble<T> {
...
#[derive(Debug)]
pub struct SelectionStatistics<T: Float + Debug + Send + Sync + 'static> {
...
impl<T: Float + Debug + Send + Sync + 'static> SelectionStatistics<T> {
    pub fn new() -> Self {
        Self {
            total_selections: 0,
...
#[derive(Debug)]
pub struct WeightUpdateMechanism<T: Float + Debug + Send + Sync + 'static> {
...
/// Ensemble optimization results
#[derive(Debug)]
pub struct EnsembleOptimizationResults<T: Float + Debug + Send + Sync + 'static> {
...
#[derive(Debug)]
pub struct AdamOptimizer<T: Float + Debug + Send + Sync + 'static> {
    learning_rate: T,
}
...
impl<T: Float + Debug + std::fmt::Debug + Send + Sync + 'static> AdvancedOptimizer<T> for AdamOptimizer<T> {
...
macro_rules! impl_placeholder_optimizer {
    ($name:ident, $lr:expr) => {
        ...
        impl<T: Float + Debug + std::fmt::Debug + Send + Sync + 'static> AdvancedOptimizer<T> for $name<T> {
            fn optimize_step_with_context(
                &mut self,
                parameters: &Array1<T>,
                gradients: &Array1<T>,
