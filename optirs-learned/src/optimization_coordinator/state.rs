...
#[derive(Debug, Clone)]
pub struct OptimizationContext<T: Float + Debug + Send + Sync + Debug> {
...
#[derive(Debug)]
pub struct StateManager<T: Float + Debug + Send + Sync + Debug> {
...
pub struct StateValidationRule<T: Float + Debug + Send + Sync + Debug> {
    /// Rule name
    pub name: String,
...
impl<T: Float + Debug + Send + Sync + 'static> std::fmt::Debug for StateValidationRule<T> {
...
impl<T: Float + Debug + Send + Sync + 'static> StateManager<T> {
...
impl<T: Float + Debug + Send + Sync + 'static> Default for OptimizationContext<T> {
    fn default() -> Self {
        Self {
