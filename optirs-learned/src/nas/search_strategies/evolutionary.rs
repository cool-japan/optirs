...
#[derive(Debug, Clone)]
pub struct EvolutionaryConfig<T: Float + Debug + Send + Sync + 'static> {
    /// Population size
...
/// Layer specification
#[derive(Debug, Clone)]
pub struct LayerSpec<T: Float + Debug + Send + Sync + 'static> {
    /// Layer type
...
/// Layer parameters
#[derive(Debug, Clone)]
pub enum LayerParameter<T: Float + Debug + Send + Sync + 'static> {
    /// Integer parameter
    Integer(i64),
    /// Float parameter
    Float(T),
...
#[derive(Debug)]
pub struct EvolutionarySearcher<T: Float + Debug + Send + Sync + 'static> {
    /// Configuration
...
#[derive(Debug, Clone)]
pub struct EvolutionHistory<T: Float + Debug + Send + Sync + 'static> {
...
impl<T: Float + Debug + Default + Clone + Send + Sync + 'static> EvolutionarySearcher<T> {
...
impl<T: Float + Debug + Default + Clone + Send + Sync + 'static> EvolutionHistory<T> {
    fn new() -> Self {
        Self {
...
impl<T: Float + Debug + Default + Clone + Send + Sync + 'static> Default for EvolutionaryConfig<T> {
    fn default() -> Self {
        Self {
            population_size: 50,
            num_generations: 100,
