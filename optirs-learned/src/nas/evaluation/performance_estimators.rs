...
#[derive(Debug, Clone)]
pub struct PerformanceEstimatorConfig<T: Float + Debug + Send + Sync + 'static> {
...
#[derive(Debug)]
pub struct PerformanceEstimator<T: Float + Debug + Send + Sync + 'static> {
    /// Configuration
...
/// Curve fitting model
#[derive(Debug, Clone)]
pub struct CurveFittingModel<T: Float + Debug + Send + Sync + 'static> {
    /// Model type
...
#[derive(Debug, Clone)]
pub struct EarlyStopDecision<T: Float + Debug + Send + Sync + 'static> {
...
/// Prediction accuracy statistics
#[derive(Debug, Clone)]
pub struct PredictionStats<T: Float + Debug + Send + Sync + 'static> {
...
#[derive(Debug, Clone)]
pub struct LearningCurveParams<T: Float + Debug + Send + Sync + 'static> {
    /// Initial performance
...
#[derive(Debug)]
pub struct NeuralPerformancePredictor<T: Float + Debug + Send + Sync + 'static> {
    /// Network weights
...
impl<T: Float + Debug + Default + Clone + Send + Sync + 'static> PerformanceEstimator<T> {
...
impl<T: Float + Debug + Default + Clone + Send + Sync + 'static> PredictionStats<T> {
    fn new() -> Self {
        Self {
...
impl<T: Float + Debug + Default + Clone + Send + Sync + 'static> Default for PerformanceEstimatorConfig<T> {
    fn default() -> Self {
        Self {
            min_epochs: 5,
