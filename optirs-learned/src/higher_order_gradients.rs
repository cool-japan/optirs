...
pub struct HigherOrderGradients<T: Float + Debug + Send + Sync + 'static> {
...
#[derive(Debug, Clone)]
pub struct HessianMatrix<T: Float + Debug + Send + Sync + 'static> {
...
#[derive(Debug, Clone)]
pub struct KFACFactors<T: Float + Debug + Send + Sync + 'static> {
...
#[derive(Debug, Clone)]
pub struct JacobianMatrix<T: Float + Debug + Send + Sync + 'static> {
...
#[derive(Debug, Clone)]
pub struct HigherOrderResults<T: Float + Debug + Send + Sync + 'static> {
...
impl<T: Float + Debug + Default + Clone + Send + Sync + 'static> HigherOrderGradients<T> {
    ...
    pub fn hessian_vector_product(
        ...
        let jvp = self.engine.compute_jvp(output_id, vector.as_slice().expect("unwrap failed"))?;
...
pub mod utils {
    ...
    pub fn is_positive_definite<T: Float + Debug + Send + Sync + 'static>(hessian: &HessianMatrix<T>) -> bool {
    ...
    pub fn condition_number<T: Float + Debug + Send + Sync + 'static>(eigenvalues: &Array1<T>) -> Option<T> {
    ...
    pub fn sparsity_ratio<T: Float + Debug + Send + Sync + 'static>(matrix: &Array2<T>, tolerance: T) -> f64 {
    ...
    pub fn identity_hessian<T: Float + Debug + Send + Sync + 'static>(size: usize) -> HessianMatrix<T> {
        HessianMatrix {
...
mod tests {
    ...
    fn test_utils_functions() {
        ...
        let matrix = Array2::from_shape_vec((2, 2), vec![1.0, 0.0, 0.0, 1.0]).expect("unwrap failed");
