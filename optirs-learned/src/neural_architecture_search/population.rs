...
pub struct PopulationManager<T: Float + Debug + Send + Sync + 'static> {
    /// Current population
...
impl<T: Float + Debug + Send + Sync + 'static> PopulationManager<T> {
...
#[derive(Debug, Clone)]
pub struct PopulationSnapshot<T: Float + Debug + Send + Sync + 'static> {
...
impl DiversityTracker {
    ...
    pub fn update<T: Float + Debug + Send + Sync + 'static>(&mut self, population: &[Individual<T>]) {
    ...
    fn calculate_diversity<T: Float + Debug + Send + Sync + 'static>(&self, population: &[Individual<T>]) -> f64 {
        if population.len() < 2 {
            return 0.0;
        }
