...
/// Model parameters
pub enum ModelParameters<T: Float + Debug + Send + Sync + 'static> {
    LinearRegression {
        weights: Array2<T>,
        bias: Array1<T>,
    },
...
#[derive(Debug)]
pub struct PredictorMetrics<T: Float + Debug + Send + Sync + 'static> {
...
impl<T: Float + Debug + Send + Sync + 'static> PredictorMetrics<T> {
    pub fn new() -> Self {
        Self {
            total_predictions: 0,
            total_trainings: 0,
